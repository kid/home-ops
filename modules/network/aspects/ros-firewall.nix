...
# between rb5009's own manual/local rules (device.aspect.terragruntInputs.
# firewall, see modules/devices/rb5009.nix) and the `firewall` quirk pool
# (den.quirks.firewall — network/cluster/etc.-emitted rule fragments,
# collected here via modules/den/policies/pipes.nix's device-collect-
# firewall). Quirk-derived rules are placed before each network's manual
# leftovers, matching every network's pre-hoist rule order (confirmed
# against the hand-written rb5009.nix this was split out of) — the
# Terraform module keys each rendered resource as "<chain>-%03d" by list
# position, so reordering would cause spurious destroy/recreate.
{ lib, ... }:
{
