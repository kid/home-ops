...
- `nix run .#refresh-nix-hash -- <file> <owner/repo> <rev>` — recompute the `hash` next to a `fetchFromGitHub` pin after bumping its `rev` by hand (e.g. `refresh-nix-hash modules/den/aspects/kubernetes/argocd/default.nix argoproj/argo-cd v3.4.5`). Renovate's self-hosted run (`.github/workflows/renovate.yaml`) calls this automatically via `postUpgradeTasks` for every `# renovate: datasource=github-releases` pin — the hash must change whenever `rev` does, or Nix silently keeps serving the old fetch's content.
...
- Files named `_foo.nix` (e.g. `modules/den/batteries/terragrunt/_ros-lib.nix`, `modules/den/batteries/terragrunt/_render-lib.nix`) are plain `import`ed libs, not modules — the leading underscore makes `import-tree` skip them. Don't drop the underscore, or they'll be auto-imported as broken flake-parts modules.
...
- A host-scope quirk collected via `pipe.collectAll` onto a `cluster` does **not** reach that cluster's `k8s-manifests` content functions as a special arg (confirmed empirically) — nixidy's module system consumes the den-wrapped module list verbatim, unlike the `terragrunt-stacks` class, which explicitly unwraps pipe data to plain args. `charts`/`generators`/`lib`/`pkgs`/`cluster` do reach `k8s-manifests` functions; collected quirks don't. To get per-host data into a `k8s-manifests` aspect generically, compute it in the cluster's own entity file (e.g. `modules/den/clusters/prd.nix`, which has real `config.den.hosts.<system>` access) and expose it via `den.clusters.<name>.methods.<name>` (see `sops-operator`'s `cluster.methods.mkSopsSecret` for the established pattern) — the aspect then just reads `cluster.methods.<name>`, no host names hardcoded in shared aspect code.
